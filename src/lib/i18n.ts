...
const initializeI18n = async (initialLocale?: string): Promise<typeof i18n> => {
  ...
  initializationPromise = new Promise((resolve, reject) => {
    const run = async () => {
      try {
        let startupLanguage = initialLocale || 'tr'

        if (isBrowser() && !initialLocale) {
          i18n.use(LanguageDetector)
          startupLanguage = getUserPreferredLanguage()
        }

        await i18n.use(initReactI18next).init({
          resources,
          lng: startupLanguage, // Initial language passed from server or detected
          fallbackLng: 'en',
          debug: false,
...
// Otomatik başlatmayı kaldırdık. I18nProvider üzerinden kontrol edilecek.
...
export const updateLanguageCookies = (lng: string): void => {
  try {
    ...
    console.error('Dil durumu güncelleme hatası:', error)
  }
}

// Deprecated: Just for backward compatibility if needed, but preferably use updateLanguageCookies + instance.changeLanguage
export const changeLanguage = async (lng: string): Promise<void> => {
  updateLanguageCookies(lng)
  if (i18n.isInitialized) {
