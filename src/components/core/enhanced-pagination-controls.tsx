  ...
  return (
    ...
    >
      ...
      <div className='flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-4'>
        {/* Page Navigation */}
        {/* Mobile View (< sm) */}
        <div className='flex w-full items-center justify-between px-1 sm:hidden'>
        ...
        {showQuickJumper && (
          <div className='hidden items-center gap-2 sm:flex'>
            <span className='whitespace-nowrap text-muted-foreground'>{t('common.pagination.goTo')}</span>
            <Input
              type='number'
              value={jumpPage}
  ...
  Object.entries(filters).forEach(([key, value]) => {
    if (value !== undefined && value !== null && value !== '') {
      if (Array.isArray(value)) {
        if (value.length > 0) {
          processedData = processedData.filter((item) =>
            value.includes((item as unknown as Record<string, unknown>)[key]),
          )
        }
      } else {
        processedData = processedData.filter((item) => {
